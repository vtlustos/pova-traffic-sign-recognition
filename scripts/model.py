...
# %%
detect_path = os.path.join(BASE_PATH, "detect-full", "dataset.yaml")
...
if TRAIN_YOLO and DOUBLE_STEP == False:
    ...
    results = model.train(data=detect_path, epochs=100, imgsz=640, batch=16, fliplr=0, cache=True)
    print(results)
...
# %%
if TRAIN_YOLO and DOUBLE_STEP:
    model = YOLO('yolov8m-cls.yaml')  # load a pretrained model
    results = model.train(data=classify_path, epochs=100, imgsz=128, batch=512, fliplr=0, flipud=0, scale=0, translate=0, 
                          perspective=0.5, shear=0, mosaic=0, workers=4, augment=False, cache=True,  optimizer="AdamW")
    print(results)
